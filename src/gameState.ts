import { GameType, PlayerType, ResourceObjectType, UnknownTransaction } from './types.js';
...
function clearProbabilitiesForResolvedTransaction(transaction: UnknownTransaction): void {
...
function resolveUnknownProbabilitiesForResource(resourceType: keyof ResourceObjectType): void {
  ...
  const affectedTransactions = game.unknownTransactions.filter(
    transaction => !transaction.isResolved && transaction.possibleResources[resourceType] > 0
  );
...
export function canPlayerAffordCost(
  ...
  for (const [resource, requiredAmount] of Object.entries(cost)) {
    ...
    if (required > 0) {
      ...
      if (!attemptToResolveUnknownTransactions(playerName, resourceKey, required)) {
        return false;
      }
    }
  }
